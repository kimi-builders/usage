...
export function SubscriptionPortfolioReview({ review, providers, zh, onSettings }) {
  ...
  return <section className="subscription-portfolio-review">
    ...
    <div className="portfolio-readiness">
      ...
      <article><span>{zh ? '免费 / 活动 / 单位权益' : 'NON-PAID BENEFITS'}</span><strong>{review.benefitProviders.length}</strong><p>{zh ? `近 30 天 API 等价 ${money(review.benefitApiEquivalentUsd)}` : `${money(review.benefitApiEquivalentUsd)} 30D API equivalent`}</p></article>
