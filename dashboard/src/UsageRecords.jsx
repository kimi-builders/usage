...
import { compactNumber, displayMoney, percent, pluralUnit, sourceLabel, usdMoney } from './format.js';
...
function costText(row, zh, currency) {
  ...
  return `${displayMoney(row.costMicros, currency)}${row.status === 'partial' ? '*' : ''}`;
}
...
export function RecordsSection({ report, zh, currency, device }) {
  ...
  return <section className="panel records-panel" id="records">
    <header className="panel-header"><div><h2>{zh ? '明细' : 'Records'}</h2><p>{zh ? `按 ${grain === 'day' ? '日' : '30 分钟'}×Agent×模型×推理强度×Agent 版本×项目×设备 聚合 · 共 ${rows.length.toLocaleString('zh-CN')} 组` : `${grain === 'day' ? 'Daily' : '30-minute'} groups · ${rows.length.toLocaleString('en-US')} ${pluralUnit(rows.length, 'group')}`}</p></div><div className="record-controls"><GrainTabs active={grain} onChange={setGrain} zh={zh} controlsId={recordsPanelId}/><ColumnsMenu enabled={enabled} setEnabled={setEnabled} zh={zh}/></div></header>
    <div id={recordsPanelId} role="tabpanel" aria-label={grain === 'day' ? (zh ? '按日聚合明细' : 'Daily grouped records') : (zh ? '30 分钟聚合明细' : '30-minute grouped records')}>{visible.length ? <>
      <div className="records-scroll"><table><thead><tr><th>{grain === 'day' ? (zh ? '日期' : 'Day') : (zh ? '时间' : 'Time')}</th><th>Agent</th><th>{zh ? '模型' : 'Model'}</th>{enabled.includes('project') ? <th>{zh ? '项目' : 'Project'}</th> : null}{enabled.includes('device') ? <th>{zh ? '设备' : 'Device'}</th> : null}{enabled.includes('effort') ? <th>{zh ? '推理强度' : 'Effort'}</th> : null}{enabled.includes('agentVersion') ? <th>{zh ? 'Agent 版本' : 'Agent ver.'}</th> : null}{enabled.includes('provider') ? <th>{zh ? '供应方' : 'Provider'}</th> : null}<th className="numeric">{zh ? '输入(含缓存写)' : 'Input+CW'}</th>{enabled.includes('cacheWrite') ? <th className="numeric">{zh ? '缓存写' : 'Cache W'}</th> : null}<th className="numeric">{zh ? '缓存读' : 'Cache R'}</th><th className="numeric">{zh ? '输出' : 'Output'}</th>{enabled.includes('reasoning') ? <th className="numeric">{zh ? '推理' : 'Reasoning'}</th> : null}<th className="numeric">{zh ? '总 TOKEN' : 'Total'}</th><th className="numeric">{zh ? '命中率' : 'Hit%'}</th><th className="numeric">{zh ? '请求' : 'Reqs'}</th><th className="numeric">{zh ? 'API 等价' : 'API equiv.'}</th></tr></thead><tbody>{visible.map((row) => <tr key={row.id}><td>{timeText(row)}</td><td><span className="agent-chip"><ToolGlyph id={row.source} size={12}/>{sourceLabel(row.source)}</span></td><td><b>{row.modelCanonical || row.model}</b>{row.modelCanonical && row.modelCanonical !== row.model ? <small>{row.model}</small> : null}</td>{enabled.includes('project') ? <td>{row.project || '•••'}</td> : null}{enabled.includes('device') ? <td>{device}</td> : null}{enabled.includes('effort') ? <td>{row.reasoningEffort || '—'}</td> : null}{enabled.includes('agentVersion') ? <td>{row.agentVersion || '—'}</td> : null}{enabled.includes('provider') ? <td>{row.modelProvider || '—'}</td> : null}<td className="numeric">{compact(row.inputTokens + row.cacheWriteInputTokens)}</td>{enabled.includes('cacheWrite') ? <td className="numeric">{compact(row.cacheWriteInputTokens)}</td> : null}<td className="numeric">{compact(row.cacheReadInputTokens)}</td><td className="numeric">{compact(row.outputTokens)}</td>{enabled.includes('reasoning') ? <td className="numeric">{compact(row.reasoningOutputTokens)}</td> : null}<td className="numeric">{compact(row.totalTokens)}</td><td className="numeric"><span className="hit-chip">{hitRate(row) == null ? '—' : percent(hitRate(row))}</span></td><td className="numeric">{compact(row.requestCount)}</td><td className={row.status === 'unpriced' ? 'numeric' : 'numeric green'}>{costText(row, zh, currency)}</td></tr>)}</tbody></table></div>
