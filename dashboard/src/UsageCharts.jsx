...
function costText(micros) { return usdMoney(micros); }
...
function metricText(row, metric, zh) {
  if (metric === 'cost') return costText(row.costMicros || 0);
...
function axisText(value, metric) {
  if (value === 0) return '0';
  if (metric === 'cost') return costText(value);
  if (metric === 'duration') {
...
function trendFacts(row, zh) {
  const facts = [
    `${compact(row.requestCount || 0)} ${zh ? '次请求' : 'requests'}`,
    `${zh ? '标准 API 等价估算' : 'Standard API-equivalent estimate'} ${costText(row.costMicros || 0)}`,
...
function trendPointLabel(row, metric, zh) {
  const tokens = [
  ...
  return `${row.label}: ${metricText(row, metric, zh)}; ${tokens.join('; ')}; ${trendFacts(row, zh)}`;
}

function TrendCore({ rows, metric, zh, average = true, plotHeight = 192, tooltipTitle, tooltipNote }) {
  ...
  return <div className="trend-core" ref={viewport} onMouseLeave={() => setHovered(null)}>
    <div className="trend-svg-scroll"><div style={{ minWidth: Math.min(560, width) }}><div className="trend-svg-stage" style={{ maxWidth: width }}>
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={zh ? '趋势图' : 'Trend chart'}>
        {ticks.map((tick, index) => <g key={index}><line x1={padL} y1={y(tick)} x2={width - padR} y2={y(tick)} className="trend-grid-line"/><text x={padL - 8} y={y(tick) + 3.5} textAnchor="end" className="trend-axis-text">{axisText(tick, metric)}</text></g>)}
      ...
      <div className="trend-hit-zones" style={{ left: `${padL / width * 100}%`, top: `${padT / height * 100}%`, width: `${plotWidth / width * 100}%`, height: `${plotHeight / height * 100}%` }}>{rows.map((row, index) => <button ref={(node) => { pointRefs.current[index] = node; }} key={row.key} type="button" tabIndex={index === rovingIndex ? 0 : -1} aria-label={trendPointLabel(row, metric, zh)} onKeyDown={(event) => onPointKeyDown(event, index)} onMouseEnter={(event) => setHovered({ index, left: tooltipLeft(event, viewport.current) })} onFocus={(event) => { setFocusIndex(index); setHovered({ index, left: tooltipLeft(event, viewport.current) }); }} onBlur={() => setHovered(null)}/>)}</div>
    </div></div></div>
    {active && hovered ? <aside className="trend-tooltip" role="tooltip" style={{ left: hovered.left }}><strong>{tooltipTitle ? tooltipTitle(active) : active.label}</strong><div className="trend-tooltip-total"><span>{metricText(active, metric, zh)}</span>{metric === 'tokens' ? <small>{zh ? '命中率' : 'hit'} {cacheHit(active)}</small> : null}</div>{tooltipNote?.(active, hovered.index)}<TokenBreakdown row={active} zh={zh}/><footer>{trendFacts(active, zh)}</footer></aside> : null}
  </div>;
}
...
export function DailyTrend({ report, zh, metric, onMetric }) {
  ...
  return <div className="activity-layout" id="activity">
    <section className="panel heatmap-panel" onMouseLeave={() => setHovered(null)}>
      ...
      {hovered && selectedCell ? <aside className="heatmap-tooltip" role="tooltip">
        ...
        <footer>{zh ? '标准 API 等价估算' : 'Standard API-equivalent estimate'} {costText(selectedCell.costMicros)} · {zh ? '活跃' : 'Active'} {duration(selectedCell.activeSeconds, zh)} · {selectedCell.userMessageCount} {zh ? '条用户消息' : 'user messages'}</footer>
      </aside> : null}
...
export function DistributionCard({ type, rows, zh }) {
  ...
  return <section className="panel distribution-card"><header className="panel-header"><h2><Icon size={14}/>{labels[type]}</h2><MetricTabs active={metric} onChange={setMetric} zh={zh} items={DIST_METRICS} className="tiny-tabs" controlsId={metricPanelId}/></header><div className="distribution-list" id={metricPanelId} role="tabpanel" aria-labelledby={`${metricPanelId}-${metric}-tab`} tabIndex={0}>{shown.length ? shown.map((row, index) => {
    ...
    return <div className="distribution-row" key={row.id}><span className="rank">{String(index + 1).padStart(2, '0')}</span><span className="distribution-name">{type === 'source' ? <ToolGlyph id={row.id} size={14}/> : null}<b title={row.label || row.id}>{type === 'source' ? sourceLabel(row.id) : row.label || row.id || (zh ? '未上传' : 'Not uploaded')}</b></span><span className="distribution-value">{metric === 'cost' ? costText(row.costMicros) : compact(row.totalTokens)} <small>{percent(share)}</small>{metric === 'tokens' ? <em>{costText(row.costMicros)}</em> : null}</span><span className="distribution-track"><i style={{ width: `${Math.max(2, share * 100)}%` }}/></span></div>;
