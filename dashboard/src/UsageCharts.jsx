...
function TrendCore({ rows, metric, zh, currency, average = true, plotHeight = 192, tooltipTitle, tooltipNote }) {
  ...
  const peakIndex = rows.reduce((best, row, index) => (
    best < 0 || metricValue(row, metric) > metricValue(rows[best], metric) ? index : best
  ), -1);
  const peakValue = peakIndex >= 0 ? metricValue(rows[peakIndex], metric) : 0;
  ...
  const padL = metric === 'cost' ? 78 : 64; const padR = 18; const padT = 20; const padB = 28;
  ...
  return <div className={`trend-core${hovered ? ' is-tooltip-open' : ''}`} ref={viewport} onMouseLeave={() => setHovered(null)}>
    <div className="trend-svg-scroll"><div style={{ minWidth: Math.min(560, width) }}><div className="trend-svg-stage" style={{ maxWidth: width }}>
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={zh ? '趋势图' : 'Trend chart'}>
        ...
        {rows.map((row, index) => {
          ...
          const barHeight = value <= 0 ? 1 : Math.max(2, value / max * plotHeight);
        ...
        {peakIndex >= 0 ? <text x={padL + peakIndex * slot + slot / 2} y={Math.max(10, y(peakValue) - 6)} textAnchor="middle" className="trend-peak-label">{axisText(peakValue, metric, currency, zh)}</text> : null}
      ...
      <div className="trend-hit-zones" style={{ left: `${padL / width * 100}%`, top: `${padT / height * 100}%`, width: `${plotWidth / width * 100}%`, height: `${plotHeight / height * 100}%` }}>{rows.map((row, index) => <button ref={(node) => { pointRefs.current[index] = node; }} key={row.key} type="button" tabIndex={index === rovingIndex ? 0 : -1} aria-label={trendPointLabel(row, metric, zh, currency)} onClick={goToEvidence} onKeyDown={(event) => onPointKeyDown(event, index)} onMouseEnter={(event) => setHovered({ index, ...tooltipPosition(event, viewport.current) })} onFocus={(event) => { setFocusIndex(index); setHovered({ index, ...tooltipPosition(event, viewport.current) }); }} onBlur={() => setHovered(null)}/>)}</div>
    </div></div></div>
    {active && hovered ? <aside className="trend-tooltip" role="tooltip" style={{ left: hovered.left, top: hovered.top, '--tooltip-arrow-left': `${hovered.arrowX}px` }}><strong>{tooltipTitle ? tooltipTitle(active) : active.label}</strong><div className="trend-tooltip-total"><span>{metricText(active, metric, zh, currency)}</span>{metric === 'tokens' ? <small>{zh ? '命中率' : 'hit'} {cacheHit(active)}</small> : null}</div>{tooltipNote?.(active, hovered.index)}<TokenBreakdown row={active} zh={zh}/><footer>{trendFacts(active, zh, currency)}</footer></aside> : null}
  </div>;
}
...
export function DailyTrend({ report, zh, currency, metric, onMetric }) {
  ...
  return <section className="panel trend-panel" id="trend"><header className="panel-header"><div><h2>{title}</h2><p>{zh ? `来源：本机 Collector · ${timezoneLabel()} · 30 分钟事实桶聚合` : `Source: local Collector · ${timezoneLabel()} · 30-minute fact buckets`}</p></div><div className="chart-controls">{metric === 'tokens' ? <div className="chart-legend"><span><i style={{ background: COLORS.input }}/>{zh ? '总 Token' : 'Total tokens'}</span><span><i className="dash"/>{zh ? '7 日均值' : '7-slot avg'}</span></div> : null}<MetricTabs active={metric} onChange={onMetric} zh={zh} controlsId={metricPanelId}/></div></header><div className="trend-chart" id={metricPanelId} role="tabpanel" aria-labelledby={`${metricPanelId}-${metric}-tab`} tabIndex={0}><TrendCore rows={report.series} metric={metric} zh={zh} currency={currency}/></div></section>;
}
...
export function ActivityHeatmap({ report, data, zh, currency, metric, onMetric }) {
  const metricPanelId = useId();
  const heatmapPanel = useRef(null);
