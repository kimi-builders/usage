...
export function SubscriptionCenter({ data, usageData, settings, loading, error, onRefresh, onSettings, view = 'overview', zh }) {
  ...
  return <section className="subscription-center" id="subscriptions">
    <section className="subscription-overview-grid">
      ...
      <article><span>{zh ? '已关联本机 TOKEN' : 'LINKED LOCAL TOKENS'}</span><strong>{compact(insights.summary.trackedTokens)}</strong><p>{zh ? `${insights.summary.trackedProviders} 个账户有本机用量` : `${insights.summary.trackedProviders} accounts have local usage`}</p></article>
