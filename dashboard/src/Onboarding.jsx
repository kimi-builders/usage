...
export function onboardingTotals(snapshot) {
  const buckets = snapshot?.buckets || [];
  const sessions = snapshot?.sessions || [];
  return {
    sources: new Set([...buckets, ...sessions].map((row) => row.source)).size,
    incomplete: (snapshot?.sources || []).filter((source) => ['partial', 'failed'].includes(source.status)).length,
...
export function Onboarding({ control, zh, onLocale, onControlAction, onScan, onSyncAction, onFinish }) {
  ...
  const summary = useMemo(() => onboardingTotals(snapshot), [snapshot]);
  ...
  return <main className="onboarding-page">
    ...
    <section className="onboarding-shell">
      ...
      {step === 'results' ? <section className="onboarding-card onboarding-results"><header><div><p className="section-eyebrow">STEP 03 · SCAN COMPLETE</p><h2><CheckCircle2 size={21}/>{zh ? '本机用量视图已就绪' : 'Your local usage view is ready'}</h2><p>{zh ? '这些结果目前只用于当前设备上的看板。' : 'These results are currently used only by the dashboard on this device.'}</p></div></header>{summary.incomplete ? <p role="status" className="onboarding-error">{zh ? `${summary.incomplete} 个来源扫描不完整。以下仅统计已通过校验的数据；进入看板后可在“本机与数据源”查看诊断。` : `${summary.incomplete} sources are incomplete. Counts include validated data only; see Local device & sources for diagnostics.`}</p> : null}<div className="onboarding-result-grid"><article><strong>{summary.sources}</strong><span>{zh ? '有数据的 Agent' : 'Agents with data'}</span></article><article><strong>{summary.buckets.toLocaleString()}</strong><span>30m buckets</span></article><article><strong>{summary.sessions.toLocaleString()}</strong><span>{zh ? '会话' : 'Sessions'}</span></article></div><div className="onboarding-choice"><button type="button" className="choice-card" disabled={Boolean(busy)} onClick={() => finish({ keepLocalOnly: true })}><HardDrive size={20}/><b>{busy === 'finish' ? (zh ? '正在进入…' : 'Opening…') : (zh ? '先只看本机' : 'Stay local for now')}</b><span>{zh ? '直接进入看板，以后仍可连接社区。' : 'Open the dashboard; connect later anytime.'}</span><ArrowRight size={15}/></button><button type="button" className="choice-card primary" disabled={Boolean(busy)} onClick={() => setStep(connected ? 'sync' : 'connect')}><Cloud size={20}/><b>{zh ? '连接社区看板' : 'Connect community dashboard'}</b><span>{zh ? '你可以精确选择哪些 Agent 可以同步。' : 'Choose exactly which agents may sync.'}</span><ArrowRight size={15}/></button></div>{error ? <p className="onboarding-error">{error}</p> : null}</section> : null}

      {step === 'connect' ? <section className="onboarding-card onboarding-connect"><header><div><p className="section-eyebrow">STEP 04 · OPTIONAL COMMUNITY</p><h2>{zh ? '连接你的社区账户' : 'Connect your community account'}</h2><p>{zh ? '远程数据属于你：只有随后标为“本机并同步”的 Agent 会上传；你可随时停止、断开或删除当前设备的云端数据。' : 'You own the remote data. Only agents later marked “Local + sync” upload; you can stop, disconnect, or delete this device’s cloud data anytime.'}</p></div></header>{authorization ? <DeviceAuthorizationCard authorization={authorization} zh={zh} onCancel={cancelConnection} onRetry={startConnection}/> : <div className="connect-explainer"><Cloud size={24}/><div><b>{zh ? '安全设备授权' : 'Secure device authorization'}</b><p>{zh ? '无需复制 API Key。浏览器登录社区并批准当前设备，凭据只保存在 owner-only 本机配置文件中。批准连接不会自动上传；下一步仍由你选择同步范围。' : 'No API key copying. Sign in and approve this device; its credential stays in an owner-only local config file. Approval does not upload anything; you choose the sync scope next.'}</p></div><button type="button" className="primary-btn" onClick={startConnection} disabled={busy === 'connect'}>{busy === 'connect' ? <LoaderCircle className="spin" size={15}/> : <Cloud size={15}/>} {zh ? '打开浏览器连接' : 'Connect in browser'}</button></div>}<footer><button type="button" className="ghost-btn" disabled={Boolean(busy)} onClick={() => finish({ keepLocalOnly: true })}>{zh ? '暂不连接' : 'Not now'}</button></footer>{error ? <p className="onboarding-error">{error}</p> : null}</section> : null}
