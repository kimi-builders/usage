...
function cacheWritePriceLabel(bucket, zh) {
  if (bucket.priceCacheWrite != null) return `$${bucket.priceCacheWrite}/M`;
  if (bucket.priceInput == null || !bucket.pricePattern) return '—';
  return `$${bucket.priceInput}/M · ${zh ? '同输入' : 'same as input'}`;
}

export function MethodDialog({ open, onClose, zh, data, report, currency = 'usd' }) {
  const rows = modelPriceRows(report);
  const hasProvisionalPrice = rows.some(({ bucket }) => bucket.priceProvisional);
