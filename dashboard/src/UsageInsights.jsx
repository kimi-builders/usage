...
export function UsageInsightAlerts({ budget, spikes, milestones, zh, onEditBudget }) {
  ...
  useEffect(() => {
    ...
    writeList(MILESTONE_STORAGE_KEY, [...seen, ...candidates.map((row) => row.signature)]);
  }, [milestones]);

  const budgetVisible = budget?.overPace && !dismissedBudgets.includes(budget.signature);
  const spike = useMemo(() => [...(spikes?.hourly || []), ...(spikes?.sessions || [])]
  ...
  return <div className="usage-insight-alerts" aria-live="polite">
    ...
    {celebration ? <section className="usage-insight-alert celebration"><Sparkles size={19}/><div><b>{zh ? `新里程碑：终身累计 ${compact(celebration.value)} Token` : `New milestone: ${compact(celebration.value)} lifetime Token`}</b><p>{zh ? `你在 ${celebration.crossedOn} 跨过了新的本机用量里程碑。数据来自未经筛选的完整本地历史。` : `You crossed a new local-usage milestone on ${celebration.crossedOn}. This uses complete, unfiltered local history.`}</p></div>{closeButton(zh ? '关闭里程碑祝贺' : 'Dismiss milestone celebration', () => setClosedCelebrations((current) => [...current, celebration.signature]))}</section> : null}
  </div>;
}
...
export function UsageInsightSummary({ budget, milestones, spikes, zh, onEditBudget }) {
  ...
  return <section className="usage-insight-summary" aria-label={zh ? '个人目标与里程碑' : 'Personal target and milestones'}>
    ...
    <article className="milestone-summary-card">
      ...
      <div className="milestone-summary-main"><div className="streak-value"><Flame size={21}/><strong>{milestones.streak}</strong><span>{zh ? '天连续构建' : 'day build streak'}</span></div><div className="milestone-next"><p><b>{compact(milestones.lifetimeTokens)}</b><span>{milestones.nextMilestone ? ` / ${compact(milestones.nextMilestone)}` : (zh ? ' · 已完成全部里程碑' : ' · all milestones reached')}</span></p><div className="insight-progress" role="progressbar" aria-label={zh ? '下一 Token 里程碑进度' : 'Next Token milestone progress'} aria-valuemin="0" aria-valuemax="100" aria-valuenow={Math.round(milestones.progress * 100)}><i style={{ width: `${milestones.progress * 100}%` }}/></div><small>{milestones.peakDay ? (zh ? `单日最高 ${compact(milestones.peakDay.totalTokens)} · ${milestones.peakDay.key}` : `Peak day ${compact(milestones.peakDay.totalTokens)} · ${milestones.peakDay.key}`) : (zh ? '等待第一天本机用量' : 'Waiting for the first local usage day')}</small></div></div>
      <footer><span>{zh ? '已达成' : 'Achieved'}</span>{milestones.achieved.length ? milestones.achieved.map((value) => <b key={value}>{compact(value)}</b>) : <small>{zh ? '下一个：1B' : 'Next: 1B'}</small>}{spikes?.status === 'building' ? <em>{zh ? `激增基线 ${spikes.sampleDays}/7 天` : `Spike baseline ${spikes.sampleDays}/7 days`}</em> : null}</footer>
    </article>
  </section>;
}
