...
import { c, formatNumber } from './cli-ui.js';

export async function runInspect() {
  const config = loadConfig();
  const snapshot = await collectLocalSnapshot({ config });
  console.log(`${c.bold(c.cyan('Kimi Builders Usage'))} · ${c.dim('dry run（未联网）')}\n`);
  console.log(c.bold('来源与读取目录:'));
  for (const source of snapshot.sources) {
    if (source.roots.length === 0) {
      console.log(`  ${c.dim(source.source)}: ${c.gray('未检测到')}`);
      continue;
    }
    for (const root of source.roots) console.log(`  ${c.bold(source.source)}: ${c.dim(root)}`);
    if (source.status === 'failed') console.log(`    → ${c.red(`解析失败：${source.error}`)}`);
  ...
  console.log(`  project: ${c.green('不上传（默认）')}`);
  console.log(`  prompt/response/tool result/full path/provider credential: ${c.green('永不上传')}`);
  console.log(`\n检测结果: ${c.bold(c.cyan(`${snapshot.summary.bucketCount} buckets`))} · ${c.bold(c.cyan(`${snapshot.summary.sessionCount} sessions`))}`);
  if (snapshot.diagnostics.rejected.length > 0) {
    console.log(`协议校验: ${c.yellow(`${snapshot.diagnostics.rejected.length} 条异常记录已隔离`)}`);
  }
}
