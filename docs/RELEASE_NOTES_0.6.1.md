...
- 计量可信度依据 `measurement` 字段，与定价假设分离；unknown 不再算作已知归因。
- 预算和其他偏好保存在本机，跨端口重启保留；保存失败会明确提示。
- 只有主动检测登录才探测本机凭据；隐私说明明确“读取日志、提取字段、不保留正文”。
- 本机 JSON `factSchemaVersion: 2` 增补缓存 TTL、会话小时片段和独立盐化标识；社区协议保持 v2。
- 首次扫描范围不变时直接复用结果；daemon 保留白名单来源目录环境变量，拒绝控制字符。
- npm 包附完整第三方许可证，包审计阻止遗漏或过期的文本。

- 大历史读取：Kimi、Pi、Gemini JSONL 逐行读取且限制单记录；Codex 仅保留必要上下文字段，及时释放重复副本。全量扫描仍会读取历史，checkpoint 用于同步去重，不是文件偏移缓存。
