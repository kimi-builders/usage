...
Collector 必须打开并解码已选日志记录，其中可能含有对话正文和完整工作目录路径。
解析器只提取白名单用量字段，丢弃其余字段；正文和完整路径不保存在看板事实、导出或
社区 payload 中。Kimi、Pi 与 Gemini 的 JSONL 逐行处理，单记录上限 16 MiB；超限或损坏会
明确标记部分扫描，不能当作完整成功。

预算目标、主题、已关闭提醒和海报身份/头像保存在 owner-only 本机 `preferences.json`，
与来源策略及社区配置分离，切换本地端口后仍保留，且不参与社区同步。当前浏览器旧值
只在本机尚无对应值时迁移；无法自动发现其他旧端口里的浏览器设置。用户填写的海报
身份仅进入主动导出的海报；本机 JSON 事实使用从安装级哈希派生的独立会话标识，
不暴露原始 session ID，也不复用社区同步的 session hash。

普通打开看板只读取设置，不探测登录文件或钥匙串内容。用户在权益设置中点击
“检测本机登录”后才执行本机凭据检测。保存并启用额度查询后，已选平台可以使用凭据
查询额度；仅检测登录不会请求供应商接口。
